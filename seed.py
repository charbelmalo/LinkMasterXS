...
def seed_database():
    ...
    with app.app_context():
        ...
        if not test_user:
            ...
            db.session.commit()
            print(f"Created test user: {test_username} with email: {test_email}")
        else:
        ...
        for data in shortcuts_data:
            ...
            existing_shortcut = Shortcut.query.filter_by(id=data.get('id')).first()
            if existing_shortcut:
                print(f"Shortcut with ID {data.get('id')} already exists. Skipping.")
                continue
